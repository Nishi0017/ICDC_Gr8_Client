...
// ===== inputMapping 共通化 =====
window.inputMapping = window.inputMapping || Array(9).fill(null);
const inputMapping = window.inputMapping;

// ===== パッド順序 =====
const padOrder = [0,1,2,3,4,5,6,7,8]; // 光らせる順番
const requiredPads = numPlayers * Math.floor(9 / PadNumMap[game]);
...
function createPadGrid(playerNames = []) {
  ...
  if (PadNum === 1) {
    ...
    namePositions = [0, 4];
    padSymbols = { 0: "↔", 4: "↔", 1: "←", 5: "←", 2: "→", 6: "→", 3: "S", 7: "S" };
  } else if (PadNum === 3) {
    if (numPlayers === 3) {
      matContainer.style.gridTemplateColumns = "repeat(11, 80px)";
      layout = ["",0,"","","",3,"","","",6,"",1,"e",2,"",4,"e",5,"",7,"e",8];
      namePositions = [0,3,6];
    } else if (numPlayers === 2) {
      matContainer.style.gridTemplateColumns = "repeat(7, 80px)";
      layout = ["",0,"","","",3,"",1,"e",2,"",4,"e",5];
      namePositions = [0,3];
    }
    padSymbols = { 0:"S",3:"S",6:"S",1:"←",4:"←",7:"←",2:"→",5:"→",8:"→" };
  } else if (PadNum === 4) {
    if (numPlayers === 4) {
      matContainer.style.gridTemplateColumns = "repeat(7, 80px)";
      layout = [0,"e",1,"",2,"e",3,"","","","","","",4,"e",5,"",6,"e",7];
      namePositions = [0,2,4,6];
    } else if (numPlayers === 3) {
      matContainer.style.gridTemplateColumns = "repeat(11, 80px)";
      layout = [0,"e",1,"",2,"e",3,"",4,"e",5];
      namePositions = [0,2,4];
    } else if (numPlayers === 2) {
      matContainer.style.gridTemplateColumns = "repeat(7, 80px)";
      layout = [0,"e",1,"",2,"e",3];
    ...
    if (game === "basketball-game") {
      padSymbols = {0:"↔",2:"↔",4:"↔",6:"↔",1:"S",3:"S",5:"S",7:"S",8:""};
    } else if (game === "snake-game") {
      padSymbols = {0:"←",2:"←",4:"←",6:"←",1:"→",3:"→",5:"→",7:"→",8:""};
    }
  }
  ...
  layout.forEach(v=>{
    ...
    if(v===""||v===" ") pad.classList.add("empty");
    else if(typeof v==="string" && v==="e"){ pad.classList.add("extra"); pad.textContent="E"; }
    else { pad.dataset.index=v; pad.textContent = padSymbols[v]||""; }
...
function highlightPad(idx){
  document.querySelectorAll(".pad[data-index]").forEach(p=>{
    ...
    if(parseInt(p.dataset.index)===idx) p.classList.add("active");
  });
}
...
function finishConfig(){
  for(let i=0;i<inputMapping.length;i++){
    if(inputMapping[i]===null) inputMapping[i]=currentConfigIndex++;
  }
...
function startConfigStep(){
  if(currentConfigIndex>=usedPadOrder.length){ finishConfig(); return; }
...
function handlePadInput(states){
  ...
  if(currentConfigIndex>=usedPadOrder.length) finishConfig();
  else startConfigStep();
}

// ===== ESP32/MQTT入力対応 (修正) =====
document.addEventListener("menu-virtual-key",(ev)=>{
  const states = ev.detail?.states;
